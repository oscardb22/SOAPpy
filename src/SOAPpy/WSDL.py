    ...
    def __init__(self, wsdlsource, config=Config, **kw):
        ...
        if self.wsdl is None and hasattr(wsdlsource, "read"):
            ...
            try:
                ...
                newstream = urllib.request.URLopener(key_file=config.SSL.key_file, cert_file=config.SSL.cert_file).open(
                    wsdlsource)
                buf = newstream.readlines()
                raise Exception("Unable to parse WSDL file at %s: \n\t%s" % (wsdlsource, "\t".join(buf)))
        ...
        if self.wsdl is None:
            try:
                stream = urllib.request.URLopener(
                    key_file=config.SSL.key_file, cert_file=config.SSL.cert_file).open(wsdlsource)
        ...
        self.soapproxy = SOAPProxy('http://localhost/dummy.webservice', config=config, **kw)
